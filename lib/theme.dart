...
class AppTextStyles {
  // ━━━━━━━━━━━━━━━━━━━━━━━ UI styles (Tajawal) ━━━━━━━━━━━━━━━━━━━━━━━
  // Tajawal: خط نسخ حديث، حروفه منفصلة وواضحة، صُمم خصيصاً للشاشات.
  // letterSpacing: 0 (Tajawal بطبيعته متباعد)
  // height: 1.7-1.9 لمنع تداخل السطور

  static TextStyle header({Color? color}) => GoogleFonts.tajawal(
        fontSize: 24,
  ...
  static TextStyle subheader({Color? color}) => GoogleFonts.tajawal(
        fontSize: 18,
  ...
  /// نسخة بدون تشكيل كثيف للأذكار القصيرة (Tajawal أوضح).
  static TextStyle dhikr({Color? color, double? fontSize}) => GoogleFonts.tajawal(
...
ThemeData get lightTheme => ThemeData(
      ...
      bottomNavigationBarTheme: BottomNavigationBarThemeData(
        ...
        selectedLabelStyle:
            AppTextStyles.caption(color: AppColors.primaryTeal),
      ...
      textTheme: TextTheme(
        ...
        labelLarge: AppTextStyles.button(color: AppColors.primaryTeal),
      ),
...
ThemeData get darkTheme => ThemeData(
      ...
      textTheme: TextTheme(
        ...
        labelLarge: AppTextStyles.button(color: AppColors.darkTeal),
      ),
      inputDecorationTheme: InputDecorationTheme(
        filled: true,
        ...
        border: OutlineInputBorder(
          borderRadius: BorderRadius.circular(AppRadius.md),
          borderSide: const BorderSide(color: AppColors.textOnDarkSecondary),
        ),
        enabledBorder: OutlineInputBorder(
          borderRadius: BorderRadius.circular(AppRadius.md),
          borderSide: const BorderSide(color: AppColors.textOnDarkSecondary),
...
class ThemeProvider extends ChangeNotifier {
  static const _kThemeKey = 'theme_mode'; // 0=system, 1=light, 2=dark
  ...
  Future<void> load() async {
    try {
      ...
      final stored = prefs.getInt(_kThemeKey) ?? 0;
      _themeMode =
          ThemeMode.values[stored.clamp(0, ThemeMode.values.length - 1)];
    } catch (_) {
  ...
  Future<void> _persist() async {
    try {
      final prefs = await SharedPreferences.getInstance();
      await prefs.setInt(_kThemeKey, _themeMode.index);
    } catch (_) {/* best-effort */}
  }
