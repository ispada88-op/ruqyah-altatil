...
**لرفع التطبيق على منصة أبل (App Store) من دون Mac:** راجع الدليل المفصّل **ارفع_التطبيق_منصة_ابل.md** (إنشاء Bundle ID، App Store Connect، مفتاح API، Codemagic، TestFlight، وإرسال المراجعة).

للخطوات المتبقية (GitHub، Codemagic، روابط المعاينة) راجع ملف **متبقي_عليك.md** في جذر المشروع.
